...
class Checkpointer():
    def __init__(self, output_dir=None, file_name='amdim_cpt.pth'):
    ...
    def restore_model_from_checkpoint(self, cpt_path, training_classifier=False):
        ckp = torch.load(cpt_path, map_location=torch.device("cuda"))
        hp = ckp['hyperparams']
        params = ckp['model']
        ...
        self.model = Model(ndf=hp['ndf'], n_rkhs=hp['n_rkhs'],
    ...
    def _save_cpt(self):
        cpt_path = os.path.join(self.output_dir, self.file_name)
