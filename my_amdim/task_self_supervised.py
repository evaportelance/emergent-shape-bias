...
from costs import loss_xent, loss_class
...
#        for _, ((images1, images2), labels) in enumerate(train_loader):
        for _, ((images1, images2), (colours, shapes, materials, sizes)) in enumerate(train_loader):

            # get data and info about this minibatch
#            labels = torch.cat([labels, labels]).to(device)
            colours = torch.cat([colours, colours]).to(device)
            shapes = torch.cat([shapes, shapes]).to(device)
            materials = torch.cat([materials, materials]).to(device)
            sizes = torch.cat([sizes, sizes]).to(device)
...
#                        loss_xent(lgt_glb_lin, labels))
            ...
            loss_cls = (loss_class(eval_col, colours) +
                        loss_class(eval_sha, shapes) +
                        loss_class(eval_mat, materials) +
                        loss_class(eval_siz, sizes)
                       )
...
#            update_train_accuracies(epoch_stats, labels, lgt_glb_mlp, lgt_glb_lin)
            update_train_accuracies(epoch_stats, colours, eval_col[0], eval_col[1], 'train_acc_glb_mlp_col', 'train_acc_glb_lin_col')
            update_train_accuracies(epoch_stats, shapes, eval_sha[0], eval_sha[1], 'train_acc_glb_mlp_sha', 'train_acc_glb_lin_sha')
            update_train_accuracies(epoch_stats, materials, eval_mat[0], eval_mat[1], 'train_acc_glb_mlp_mat', 'train_acc_glb_lin_mat')
            update_train_accuracies(epoch_stats, sizes, eval_siz[0], eval_siz[1], 'train_acc_glb_mlp_siz', 'train_acc_glb_lin_siz')
...
def train_self_supervised(model, learning_rate, train_loader,
    ...
    scheduler = MultiStepLR(optimizer, milestones=[250, 400], gamma=0.2)
    #scheduler = MultiStepLR(optimizer, milestones=[3], gamma=0.2)
    epochs = 500
    #epochs = 5
    # train the model
