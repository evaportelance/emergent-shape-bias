...
class Encoder(nn.Module):
    def __init__(self, dummy_batch, num_channels=3, ndf=128, n_rkhs=1024,
...
class Evaluator(nn.Module):
    ...
    def forward(self, ftr_1, class_only=False):
        '''
        Input:
...
class Model(nn.Module):
    def __init__(self, ndf, n_rkhs, tclip=20.,
                 n_depth=3, encoder_size=32, use_bn=False, class_only=False):
        ...
        self.evaluator_col = Evaluator(8, ftr_1=rkhs_1)
        #self.evaluator_sha = Evaluator(3, ftr_1=rkhs_1)
        self.evaluator_sha = Evaluator(10, ftr_1=rkhs_1)
        self.evaluator_mat = Evaluator(2, ftr_1=rkhs_1)
        self.evaluator_siz = Evaluator(2, ftr_1=rkhs_1)


        # gather lists of self-supervised and classifier modules
        self.info_modules = [self.encoder.module, self.g2l_loss]
        self.class_modules = [self.evaluator_col, self.evaluator_sha, self.evaluator_mat, self.evaluator_siz]
...
#         return self.evaluator
    ...
    def forward(self, x1, x2, class_only=False, encoding_layer=False):
        '''
        Input:
        ...
        Output:
          ...
          OR tensor for final layer before rkhs layers when encoding_layer is True for our Vision module
        '''
        # EP: Added this so that we can return the feature tensor of the image and integrate this into our Agent models
        if encoding_layer:
            # shortcut to encode one image and evaluate classifier
            encoding, _, _ = self.encode(x1, no_grad=True)
            return encoding
...
#            lgt_glb_mlp, lgt_glb_lin = self.evaluator(rkhs_1)
            colour = self.evaluator_col(rkhs_1, class_only)
            shape = self.evaluator_sha(rkhs_1, class_only)
            material = self.evaluator_mat(rkhs_1, class_only)
            size = self.evaluator_siz(rkhs_1, class_only)

            res_dict['class'] = {"colour": colour, "shape": shape, "material": material, "size": size}
        ...
        colour = self.evaluator_col(ftr_1=torch.cat([r1_x1, r1_x2]))
        shape = self.evaluator_sha(ftr_1=torch.cat([r1_x1, r1_x2]))
        material = self.evaluator_mat(ftr_1=torch.cat([r1_x1, r1_x2]))
        size = self.evaluator_siz(ftr_1=torch.cat([r1_x1, r1_x2]))
        res_dict['class'] = {"colour": colour, "shape": shape, "material": material, "size": size}
        return res_dict
